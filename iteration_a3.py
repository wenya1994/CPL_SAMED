...
parser.add_argument('--stepsize', default=4, type=int,  #学习率衰减步长
                    metavar='SS', help='learning rate step size')
parser.add_argument('--maxepoch', default=8, type=int, metavar='N',        #最大训练轮次
...
parser.add_argument('--tmp', help='tmp folder', default='result/bsds/round2/a1+a2')  #临时文件夹路径
...
parser.add_argument('--path', default="result/bsds/round2/a1+a2/epoch-10-training-record/epoch-10-checkpoint.pth", type=str)
...
def adjust_weights_based_on_mask(weights, loss_mask,ep_mask):
    # 根据 loss_mask 调整权重
    loss_mask = loss_mask.to(device)
    ep_mask = ep_mask.to(device)
    # 根据 loss_mask 调整权重
    weights = weights.clone()  # 避免修改原始权重
    weights[(loss_mask == 5) & ((ep_mask == 1) | (ep_mask == 0))] *= 20  # 增大权重
    weights[(loss_mask == 5) & ((ep_mask == 0.098) | (ep_mask == 0.898))] *= 15
    weights[(loss_mask == 5) & (ep_mask == 0.498)] *= 10
    weights[loss_mask == 4 & ((ep_mask == 1) | (ep_mask == 0))] *= 8  # 增大权重
    weights[loss_mask == 4 & ((ep_mask == 0.098) | (ep_mask == 0.898))] *= 6  # 增大权重
    weights[(loss_mask == 4) & (ep_mask == 0.498)] *= 5
    weights[loss_mask == 3 & ((ep_mask == 1) | (ep_mask == 0))] *= 3  # 减小权重-
    weights[loss_mask == 4 & ((ep_mask == 0.098) | (ep_mask == 0.898))] *= 2  # 增大权重
    weights[loss_mask == 3 & (ep_mask == 0.498)] *= 1
    weights[loss_mask == 2 & ((ep_mask == 1) | (ep_mask == 0))] *= 0.8  # 减小权重
    weights[loss_mask == 2 & ((ep_mask == 0.098) | (ep_mask == 0.898))] *= 0.5  # 增大权重
    weights[loss_mask == 2 & (ep_mask == 0.498)] *= 0.1
    #weights[loss_mask == 1 & ((ep_mask == 1) | (ep_mask == 0))] *= 0.1  # 减小权重
...
def adjust_weights_based_on_epmask(weights, ep_mask):
    # 根据 loss_mask 调整权重
    weights = weights.clone()  # 避免修改原始权重
    weights[ep_mask == 1] *= 1# 增大权重
    weights[ep_mask == 0.8] *= 1  # 增大权重

    # 1和4的权重保持不变
    return weights

def cross_entropy_loss_RCF_update(prediction, labelef, std, ada, loss_mask, ep_mask):  # 二元交叉熵损失
    ...
    adjusted_weights = adjust_weights_based_on_mask(new_mask, loss_mask,ep_mask)
    cost = F.binary_cross_entropy(
        prediction, labelef, weight=adjusted_weights.detach(), reduction='sum')

    return cost, mask

    return cost, mask_weight


def cross_entropy_loss_RCF(prediction, labelef, std, ada,ep_mask):  # 二元交叉熵损失
    label = labelef.long()  # labelef转换为label
    mask = label.float()  # 创建mask，通过将label转换为浮点值。
    ...
    # 计算正类（1）、负类（0.9_mask_a1+a2.9_mask_a1）以及忽略类的标签数量。
    num_positive = torch.sum((mask == 1).float()).float()
    num_negative = torch.sum((mask == 0).float()).float()
    num_ignore = torch.sum(((mask - zero_5).abs() < epsilon).float()).float()

    # 确保所有标签的总数等于label张量的元素总数
    assert num_negative + num_positive + num_ignore == label.numel()

    # 根据正类和负类的数目调整了mask张量中对应位置的权重
    mask_weight = torch.ones_like(mask)
    mask_weight[mask == 1] = 1.0 * num_negative / (num_positive + num_negative)
    mask_weight[mask == 0] = 1.1 * num_positive / (num_positive + num_negative)
    mask_weight[(mask - zero_5).abs() < epsilon] = 0  # 忽略标签为忽略类的像素点

    # 将标签为忽略类的像素点排除在损失计算之外
    valid_mask = ((mask - zero_5).abs() >= epsilon).float()

    new_mask = mask_weight * torch.exp(std * ada)
    adjust_weights_based_on_epmask(new_mask, ep_mask)
    cost = F.binary_cross_entropy(prediction, labelef.float(), weight=new_mask.detach(), reduction='none')

    # 仅计算有效区域的损失
    # cost = torch.sum(cost * valid_mask) / torch.sum(valid_mask)
...
def main():
    ...
    for epoch in range(args.start_epoch, args.maxepoch):
        ...
        train(train_loader, model, optimizer, epoch,    #在一个 epoch 内使用训练数据集对模型进行训练
          save_dir=join(TMP_DIR, 'epoch-%d-training-record' % epoch), running_predictions=running_predictions, bi_dict=bi_dict, iter_begin_epoch=iter_begin_epoch)
        ...
        log.flush()  # write log
    iter(running_predictions, args.maxepoch - iter_begin_epoch, save_dir=save_iter_dir, bi_dict=bi_dict,cv_mask=cv_mask)


def iter(mask, len_epoch, save_dir, bi_dict, cv_mask,threshold=0.8,last_threshold=0.9):
    # 将 mask_a1+a2 除以 len_epoch
    save_dir1 = r"E:\UAED\6.UAED\6.UAED\txt"
    ...
    # 创建一个新的 mask_a1+a2，初始值为0
    mask_binary = torch.zeros_like(normalized_mask)

    # 将大于等于 threshold 的部分变成 0.9_mask_a1+a2.9_mask_a1.9, 代表是a2的
    #mask_binary[normalized_mask >= threshold] = 0.9
    # 大于0.8的变成 1  本来是上面这个
    mask_binary[(normalized_mask >= threshold) & (normalized_mask <=0.9) ] = 0.9
    #mask_binary[(normalized_mask >= threshold) & (normalized_mask <= 0.98)] = 0.9
    # 将小于 lower_threshold 的部分变成 0.9_mask_a1+a2.9_mask_a1.1,代表是a2的
    # mask_binary[normalized_mask < (1.0 - threshold)] = 0.1  原来是这个
    #mask_binary[(normalized_mask < (1.0 - threshold)) & (normalized_mask >= 0.02)] = 0.1
    mask_binary[(normalized_mask < (1.0 - threshold)) & (normalized_mask >=0.1)] = 0.1
    epmask_folder = r"E:\UAED\6.UAED\6.UAED\result\bsds\round2\a1+a2\mask_a22"  # 替换为你的文件夹路径
    ep_mask = load_and_stack_images(epmask_folder)
    ep_mask = ep_mask.to(mask_binary.device)

    if ep_mask.shape != mask_binary.shape:
        raise ValueError(f"ep_mask 和 mask_binary 的形状不一致：{ep_mask.shape} vs {mask_binary.shape}")

        # 创建一个条件掩码，用于选择 mask_binary 中非 0 的位置
    condition = mask_binary != 0

        # 使用 torch.where 将 mask_binary 中非 0 的值覆盖到 ep_mask 的相同位置
    updated_ep_mask = torch.where(condition, mask_binary, ep_mask)

    '''
    1.读取epmask的文件夹，拼接成1600，256，120的张量
    2.把mask_binary中非0的在epmask修改
    3.存储epmask为新的mask2
    '''
    # 保持原始0和255的像素值不变
    # mask_binary[mask == 0] = 0
    # mask_binary[mask == 1] = 1
    # mask_binary[(mask == 0) & ((cv_mask == 2) | (cv_mask == 3))] = 0
    # mask_binary[(mask == 1) & ((cv_mask == 2) | (cv_mask == 3))] = 0
    # mask_binary[(mask == 1)] = 1
    # mask_binary[(mask == 0)] = 0
    # 打印二值化后的 mask_a1+a2
    print(f"Binary mask_a1+a2: {mask_binary}")
    # 保存处理后的 mask_a1+a2
    save_mask_dir = join(save_dir, 'mask_a33')
    if not os.path.exists(save_mask_dir):
        os.makedirs(save_mask_dir)
    # 保存处理后的 mask_a1+a2
    save_mask_as_png(updated_ep_mask, save_mask_dir, bi_dict)
    # print(mask.shape)
    # print(cv_mask.shape)
    # # mask_part = mask[4, :160, :256]
    # # cv_mask_part = cv_mask[4, :160, :256]
    # # mask_binary_part = mask_binary[4, :160, :256]
    #
    # # 保存为txt文件
    # save_tensor_to_txt(mask, os.path.join(save_mask_dir, 'mask.txt'))
    # # save_tensor_to_txt(cv_mask_part, os.path.join(save_mask_dir, 'cv_mask.txt'))
    # # save_tensor_to_txt(mask_binary_part, os.path.join(save_mask_dir, 'mask_binary.txt'))
    #
    # print(f"Saved mask, cv_mask, and mask_binary parts to txt files.")
...
def train(train_loader, model, optimizer, epoch, save_dir, running_predictions, bi_dict, iter_begin_epoch):
    global cv_mask
    ...
    ###########
    ep_mask_images = get_all_ep_mask_image_paths(r'E:\UAED\6.UAED\6.UAED\result\bsds\round2\a1+a2\mask_a22')  # 自定义函数返回所有 ep_mask 图片路径
    num_ep_images = len(ep_mask_images)  # 总图片数
    images_per_batch = args.batch_size  # 每个 batch 需要 8 张图片
###################
...
#################################
        ...
        if epoch >= 0 :
            bce_loss, mask1 = cross_entropy_loss_RCF_update(outputs, label, std, ada,loss_mask,ep_mask)
        else:
            bce_loss, mask1 = cross_entropy_loss_RCF(outputs, label, std, ada,ep_mask)
        ...
        if epoch >= iter_begin_epoch:
            ...
            for j in range(args.batch_size):
                ...
                mask_condition = (ep_mask[j, 0, :, :] >= 0.498 - tolerance) & (ep_mask[j, 0, :, :] <= 0.498 + tolerance)
                # print(f"Mask condition: {mask_condition.sum()} pixels selected")

                running_predictions[idx, :_weight, :_height][mask_condition] += outputs[j, 0, :, :][mask_condition]
                # 对于0和1的像素点不变，保持原标签值
                # running_predictions[idx, :_weight, :_height][label[j, 0, :, :] == 0] = label[j, 0, :, :][
                #     label[j, 0, :, :] == 0]
                # running_predictions[idx, :_weight, :_height][label[j, 0, :, :] == 1] = label[j, 0, :, :][
                #     label[j, 0, :, :] == 1]


            #累加像素
            # for j in range(args.batch_size):
            #     name = names[j].split('/')[-1]  # 获取当前图片的文件名
            #     idx = bi_dict[name]  # 获取该图片在 running_predictions 中的索引
            #
            #     # 获取 outputs 的尺寸
            #     _, _channel, _weight, _height = outputs.shape
            #
...
def get_mask(outputs,image_name):
    ...
    img_1 = torch.zeros(height*2,width*2)
    ...
    img_1[:height, :width] = top_left  # Top-left
    img_1[height:, :width] = bottom_left  # Bottom-left
    img_1[:height, width:] = top_right  # Top-right
    img_1[height:, width:] = bottom_right  # Bottom-right

    #奇偶拼接
    img_2 = torch.zeros(height * 2, width * 2)
    ...
    padded_img_2 = torch.zeros(height * 2 + 2, width * 2 + 2)
    ...
    e =  torch.zeros(height*2,width*2)
    e[:2*height-1,:2*width-1] = edge


    e1 =  torch.zeros(height*2,width*2)
    e1[:2*height-1,:2*width-1] = edge1

    e2 =  torch.zeros(height * 2, width * 2)
    e2[:2*height-1,:2*width-1] = edge2
    e3 =  torch.zeros(height * 2, width * 2)
    e3[:2*height-1,:2*width-1] = edge3

    e4 =  torch.zeros(height * 2, width * 2)
    e4[:2*height-1,:2*width-1] = edge4
