...
parser.add_argument('--maxepoch', default=16, type=int, metavar='N',        #最大训练轮次
...
def iter(mask, len_epoch, save_dir, bi_dict, cv_mask,threshold=0.8,last_threshold=0.9):
    ...
    epmask_folder = r"E:\UAED\6.UAED\6.UAED\result\bsds\round2\a1+a2\mask_a11"  # 替换为你的文件夹路径
    ...
    # 保存处理后的 mask_a1+a2
    save_mask_dir = join(save_dir, 'mask_a22')
...
def train(train_loader, model, optimizer, epoch, save_dir, running_predictions, bi_dict, iter_begin_epoch):
    ...
    ###########
    ep_mask_images = get_all_ep_mask_image_paths(r'E:\UAED\6.UAED\6.UAED\result\bsds\round2\a1+a2\mask_a11')  # 自定义函数返回所有 ep_mask 图片路径
