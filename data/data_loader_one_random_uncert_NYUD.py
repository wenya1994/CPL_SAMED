...
class BSDS_RCFLoader(data.Dataset):
    ...
    Dataloader BSDS500
    """
    def __init__(self, root='dataset/NYUD/', split='train', transform=False):
        self.root = root
        self.split = split
        self.transform = transform
        if self.split == 'train':
            self.filelist = join(self.root, 'NYUD_train.lst')

        elif self.split == 'test':
            self.filelist = join(self.root, 'test.lst') # test不用变，一直都用这个测
        else:
    ...
    def __getitem__(self, index):
        ...
        if self.split == "train":
            ...
            lb_std, _ = pad_to_multiple_of_eight(labels.mean(dim=0).unsqueeze(0))
